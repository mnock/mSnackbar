...
import static com.mno.msnackbar.SlideUp.State.HIDDEN;
import static com.mno.msnackbar.SlideUp.State.SHOWED;
...
public class SlideUp implements View.OnTouchListener, ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    private void hide(boolean immediately) {
        if (sliderView.getHeight() <= 0) return;
        //endAnimation();
        switch (startGravity) {
            case TOP:
                if (immediately) {
    ...
    private void onAnimationUpdateStartToEnd(float value) {
        ...
        float visibleDistance = sliderView.getLeft() - sliderView.getX();
        float percents = (visibleDistance) * 100 / viewWidth;
        // e("mno"," notifyPercentChanged","onTouchStartToEnd--"+visibleDistance+"--"+sliderView.getWidth()+"---%"+percents);
        notifyPercentChanged(percents);
    }

    private void onAnimationUpdateEndToStart(float value) {
        sliderView.setTranslationX(value);
        float visibleDistance = sliderView.getX() - sliderView.getLeft();
        float percents = (visibleDistance) * 100 / viewWidth;
        notifyPercentChanged(percents);
    }
    ...
    public final void onAnimationStart(Animator animator) {
        ...
        //e("mno"," onAnimationStart",sliderView.getVisibility()+"--"+sliderView.getTranslationY()+"--"+currentState);
    }

    @Override
    public final void onAnimationEnd(Animator animator) {
        //e("mno"," onAnimationEnd",sliderView.getVisibility()+"--"+sliderView.getTranslationY()+"--"+currentState);
        if (slideAnimationTo != 0) {
            if (sliderView.getVisibility() != GONE) {
                sliderView.setVisibility(GONE);
            }
        }
        if (sliderView.getVisibility() == VISIBLE && currentState == HIDDEN)
            notifyVisibilityChanged(VISIBLE);
        if (sliderView.getVisibility() == GONE && currentState == SHOWED)
            notifyVisibilityChanged(GONE);
    }
